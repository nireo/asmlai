...
change_type(std::unique_ptr<Expression> exp, ValueT change_type,
  ...
  if (infix_opr == TokenType::Plus || infix_opr == TokenType::Minus) {
    if (number_type(exp_type) && is_ptr_type(change_type)) {
      int size =
          codegen::get_bytesize_of_type(convert_pointer_to_normal(change_type));
      if (size > 1) {
...
int compile_ast_node(const Node &node, int reg, const AstType top_type) {
  ...
  case AstType::WhileStatement: {
    ...
    codegen::gen_jmp(start_label);
    codegen::gen_label(end_label);
  ...
  case AstType::StringLiteral: {
    ...
    return codegen::load_global_str(str_lit.id_);
  }
  case AstType::IfExpression: {
    ...
    if (if_stmt.other_ != nullptr) {
      codegen::gen_jmp(end_label);
    }

    codegen::gen_label(false_label);
  ...
  case AstType::InfixExpression: {
    ...
    case TokenType::Plus:
      return codegen::add_registers(left, right);
    case TokenType::Minus:
      return codegen::sub_registers(left, right);
    case TokenType::Asterisk:
      return codegen::mul_registers(left, right);
    case TokenType::Slash:
      return codegen::div_registers(left, right);
    case TokenType::LT:
    case TokenType::EGT:
    case TokenType::ELT:
    case TokenType::GT:
    ...
    case TokenType::Neq: {
      if (top_type == AstType::IfExpression ||
        ...
        return codegen::codegen_compare_jump(left, right, reg, infix_exp.opr);
      }
      return codegen::codegen_compare_no_jump(left, right, infix_exp.opr);
    }
    case TokenType::RShift: {
      return codegen::shift_right_from_reg(left, right);
    }
    case TokenType::LShift: {
      return codegen::shift_left_from_reg(left, right);
    }
    case TokenType::LogOr: {
      return codegen::codegen_or(left, right);
    }
    case TokenType::LogAnd: {
      return codegen::codegen_and(left, right);
    }
    case TokenType::Xor: {
      return codegen::codegen_xor(left, right);
    }
    case TokenType::Assign: {
      ...
      case AstType::Identifier: {
        ...
        if (function_locals[current_function].find(identifier.value_) ==
            function_locals[current_function].end()) {
          return codegen::store_global(left, get_symbol(identifier.value_));
        } else {
          const auto &sym =
              function_locals[current_function].at(identifier.value_);
          return codegen::store_local(sym, left);
        }
      }
      case AstType::Dereference: {
        return codegen::store_dereference(left, right,
                                          infix_exp.right_->ValueType());
      }
      default: {
  ...
  case AstType::ReturnStatement: {
    ...
    codegen::codegen_return(left_reg,
                            get_symbol(returnstmt.function_identifier_));
  ...
  case AstType::FunctionLiteral: {
    ...
    codegen::function_end(sym.label);
  ...
  case AstType::TypeChangeAction: {
    ...
    case TypeChange::Scale: {
      ...
      switch (tca.size) {
      case 2:
        return codegen::shift_left(left, 1);
      case 4:
        return codegen::shift_left(left, 2);
      case 8:
        return codegen::shift_left(left, 3);
      }

      int right = codegen::codegen_load_int(tca.size);
      return codegen::mul_registers(left, right);
    }
    }
  ...
  case AstType::PrefixExpression: {
    ...
    case TokenType::Asterisk: {
      ...
      if (pref.right_->is_rvalue()) {
        return codegen::codegen_dereference(right, pref.right_->ValueType());
      }
    ...
    case TokenType::Minus: {
      ...
      return codegen::codegen_neg(right);
    }
    case TokenType::Bang: {
      int right = compile_ast_node(*pref.right_, -1, node.Type());

      return codegen::codegen_not(right);
    }
    default: {
  ...
  case AstType::IntegerLiteral: {
    ...
    return codegen::load_into_register(int_lit.value_);
  }
  case AstType::Addr: {
