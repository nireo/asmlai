...
std::unique_ptr<Statement> Parser::parse_statement() {
    switch (current_.type) {
        case TokenType::Function: return parse_function_literal();
        case TokenType::Return: return parse_return_statement();
        case TokenType::Global: return parse_global_decl();
...
static ValueT convert_type_to_pointer(const ValueT type) {
    switch (type) {
        ...
        default: std::fprintf(stderr, "cannot convert type: %d into pointer\n", type); std::exit(1);
    }
}
...
ValueT Parser::parse_type() {
    ...
    switch (peek_.type) {
        ...
        default: PARSER_ERROR("cannot parser type for token: %d \n", (int)peek_.type);
    }
...
std::unique_ptr<Expression> Parser::parse_call(std::unique_ptr<Expression> ident) {
    next_token();
    next_token();

    auto arguments_ = parse_expression_list();
    if (!current_token_is(TokenType::RParen))
        PARSER_ERROR("function call arguments needs to be wrapped in parenthesies.");
...
std::unique_ptr<Expression> Parser::parse_array(std::unique_ptr<Expression> ident) {
    next_token();
    next_token();
    ...
    if (indx->ValueType() != TYPE_INT && indx->ValueType() != TYPE_CHAR && indx->ValueType() != TYPE_LONG)
        PARSER_ERROR("the index expression needs to be an integer.");

    auto index_modified = change_type(std::move(indx), arr->ValueType(), TokenType::Plus);
...
std::unique_ptr<Expression> Parser::parse_primary() {
    ...
    switch (current_.type) {
        ...
        default:
            PARSER_ERROR("unrecognized token when parsing primary expression factor. %s",
        ...
        case TokenType::Amper: {
            ...
            if (type != TokenType::Minus && type != TokenType::Bang) {
                if (right->Type() != AstType::Identifier)
                    PARSER_ERROR("ampersand or amper cannot be used for nothing but identifiers.\n");
            }
...
std::unique_ptr<Expression> Parser::parse_while_expression() {
    ...
    if (!expect_peek(TokenType::LBrace))
        PARSER_ERROR("while statement condition should be followed by a left brace.");
...
std::unique_ptr<Expression> Parser::parse_expression_rec(Precedence prec) {
    ...
    if (current_token_is(TokenType::Semicolon) || current_token_is(TokenType::RParen)) {
    ...
    while ((prec < precedences[tokentype]) ||
        ...
        if (tokentype == TokenType::Assign) {
            ...
            auto left_temp = change_type(std::move(left), right->ValueType(), tokentype);
            auto right_temp = change_type(std::move(right), left_type, tokentype);
...
std::unique_ptr<Expression> Parser::parse_identifier() {
    ...
    if (latest_function_name != "") {
        const auto &sym = get_symbol_w_func(latest_function_name, current_.literal_);
...
std::vector<std::unique_ptr<Identifier>> Parser::parse_function_params() {
    ...
    if (!expect_peek(TokenType::RParen)) {
        PARSER_ERROR("function parameters need to be followed by right parenthesis\n.");
    }
...
std::unique_ptr<Statement> Parser::parse_var_decl() {
    ...
    if (peek_token_is(TokenType::LBracket)) {
        ...
        add_new_symbol(name, TYPE_ARRAY, convert_type_to_pointer(var_decl->type_), 0, size);
...
std::unique_ptr<Statement> Parser::parse_global_decl() {
    ...
    if (peek_token_is(TokenType::LBracket)) {
        ...
        int size = std::stoi(current_.literal_.data());
        add_new_symbol(name, TYPE_ARRAY, convert_type_to_pointer(globl->type_), 0, size);
