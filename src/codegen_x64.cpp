...
constexpr const char *b_registers[10] = {"%r10b", "%r11b", "%r12b", "%r13b", "%r9b",
                                         "%r8b",  "%cl",   "%dl",   "%sil",  "%dil"};

constexpr const char *d_registers[10] = {"%r10d", "%r11d", "%r12d", "%r13d", "%r9d",
                                         "%r8d",  "%ecx",  "%edx",  "%esi",  "%edi"};

constexpr const char *jump_insts[] = {"jne", "je", "jge", "jle", "jg", "jl"};
constexpr const char *compare_instructions[] = {"sete", "setne", "setl", "setg", "setle", "setge"};
...
int get_local_offset(ValueT type) {
    local_offset += (get_bytesize_of_type(type) > 4) ? get_bytesize_of_type(type) : 4;
    return -local_offset;
}

static int get_corresponding_inst_index(const TokenType type) {
    switch (type) {
        case TokenType::Eq: return 0;
        case TokenType::Neq: return 1;
        case TokenType::LT: return 2;
        case TokenType::GT: return 3;
        case TokenType::ELT: return 4;
        case TokenType::EGT: return 5;
        default: std::fprintf(stderr, "no free registers\n"); std::exit(1);
    }
}
...
int store_global(int r, const Symbol &sym) {
    switch (sym.value_type_) {
        case TYPE_CHAR: {
            std::fprintf(fp, "\tmovb\t%s, %s(\%%rip)\n", b_registers[r], sym.name_.c_str());
            break;
        }
        case TYPE_INT: {
            std::fprintf(fp, "\tmovl\t%s, %s(\%%rip)\n", d_registers[r], sym.name_.c_str());
            break;
        }
        case TYPE_LONG: {
            std::fprintf(fp, "\tmovq\t%s, %s(\%%rip)\n", registers[r], sym.name_.c_str());
            break;
        }
        case TYPE_PTR_CHAR:
        case TYPE_PTR_LONG:
...
void generate_sym(const Symbol &sym) {
    ...
    for (int i = 0; i < sym.size; ++i) {
        switch (size) {
            case 1: std::fprintf(fp, "\t.byte\t0\n"); break;
            case 4: std::fprintf(fp, "\t.long\t0\n"); break;
            case 8: std::fprintf(fp, "\t.quad\t0\n"); break;
            default: std::fprintf(stderr, "unrecognized byte size."); std::exit(1);
        }
    }
}
...
int load_global(const Symbol &sym, TokenType opr, bool post) {
    ...
    switch (sym.value_type_) {
        case TYPE_CHAR: {
            ...
            std::fprintf(fp, "\tmovzbq\t%s(\%%rip), %s\n", sym.name_.c_str(), registers[free_reg]);
...
int codegen_compare_no_jump(int reg1, int reg2, const TokenType type) {
    ...
    std::fprintf(fp, "\t%s\t%s\n", compare_instructions[index], b_registers[reg2]);
...
void codegen_return(int reg, const Symbol &sym) {
    switch (sym.value_type_) {
        case TYPE_CHAR: fprintf(fp, "\tmovzbl\t%s, %%eax\n", b_registers[reg]); break;
        case TYPE_INT: fprintf(fp, "\tmovl\t%s, %%eax\n", d_registers[reg]); break;
        case TYPE_LONG: std::fprintf(fp, "\tmovq\t%s, %%rax\n", registers[reg]); break;
        default: {
...
int convert_into_bool(int r, AstType node_type, int label) {
    if (node_type == AstType::IfExpression || node_type == AstType::WhileStatement) {
...
int store_dereference(int reg1, int reg2, ValueT type) {
    switch (type) {
        case TYPE_CHAR: std::fprintf(fp, "\tmovb\t%s, (%s)\n", b_registers[reg1], registers[reg2]); break;
        case TYPE_INT: std::fprintf(fp, "\tmovq\t%s, (%s)\n", registers[reg1], registers[reg2]); break;
        case TYPE_LONG: std::fprintf(fp, "\tmovq\t%s, (%s)\n", registers[reg1], registers[reg2]); break;
        default: std::fprintf(stderr, "uncompatible type for deference storing."); std::exit(1);
    }
...
int load_local(const Symbol &sym, TokenType opr, bool post) {
    ...
    switch (sym.value_type_) {
        case TYPE_CHAR: {
            ...
            std::fprintf(fp, "\tmovzbq\t%d(\%%rip), %s\n", sym.position, registers[free_reg]);
...
int store_local(const Symbol &sym, int r) {
    switch (sym.value_type_) {
        case TYPE_CHAR: std::fprintf(fp, "\tmovb\t%s, %d(%%rbp)\n", b_registers[r], sym.position); break;
        case TYPE_INT: std::fprintf(fp, "\tmovl\t%s, %d(%%rbp)\n", d_registers[r], sym.position); break;
        case TYPE_LONG:
        case TYPE_PTR_CHAR:
        case TYPE_PTR_INT:
        case TYPE_PTR_LONG: std::fprintf(fp, "\tmovq\t%s, %d(%%rbp)\n", registers[r], sym.position); break;
        default: {
